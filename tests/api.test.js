...
describe.each(tests)("Given service is running", ({ suiteName, alicePublicKey, blockchainName, data, data_burn, isValidHash }) => {
  it(`[${suiteName}] Verify that User doesn't own the token`, async () => {
  ...
  describe("When a new token is minted for User", () => {
    it(`[${suiteName}] Verify if the operation hash is valid`, async () => {
    ...
    describe("Then a new token is created", () => {
      ...
      it(`[${suiteName}] Verify if the new user owns the new token`, async () => {
        const hasResponse = await axios.get(`${SERVICE_URL}/has/${alicePublicKey}`, {
          headers: {
            "X-BLOCKCHAIN": blockchainName,
            "X-API-KEY": API_KEY
      ...
      describe("When a token is burned", () => {
        ...
        describe("Then the token is burned", () => {
          ...
          it(`[${suiteName}] Verify that the user doesn't own the token`, async () => {
            const hasResponse = await axios.get(`${SERVICE_URL}/has/${alicePublicKey}`, {
              headers: {
                "X-BLOCKCHAIN": blockchainName,
                "X-API-KEY": API_KEY
